...
namespace tateti.Filtros
{
    public class DetectorMovilFiltro : IActionFilter
    {
        ...
        public static bool EsMovil(ActionExecutedContext contexto)
        {
            var agenteUsuario = contexto.HttpContext.Request.Headers["User-Agent"].ToString();

            if(contexto.HttpContext != null && agenteUsuario != null)
            {
                if (agenteUsuario.Length < 4)
                    return false;
                if(MobileCheck.IsMatch(agenteUsuario) || MobileVersionCheck.IsMatch(agenteUsuario.Substring(0,4))) 
                    return true;
            }
            return false;
        }
        ...
        {
            var viewResult = (contexto.Result as ViewResult);
            if (viewResult == null)
                return;
            if (EsMovil(contexto))
            {
